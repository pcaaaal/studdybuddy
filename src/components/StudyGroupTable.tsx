...
export default function StudyGroupsTable({
  ...
  const handleLeaveGroup = async () => {
    ...
    try {
      const response = await deleteUserFromStudyGroup(selectedGroupId, userId);
      if (response.success) {
        console.log(`Successfully left group with ID: ${selectedGroupId}`);
        setGroups((prevGroups) => prevGroups.filter((group) => group.id !== selectedGroupId));
      } else {
        console.error(`Failed to leave group with ID: ${selectedGroupId}`);
      }
    } catch (error) {
      console.error(`Error leaving group with ID: ${selectedGroupId}`, error);
    } finally {
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg p-6 space-y-4 shadow-lg">
            ...
            <p className="text-gray-600">Do you really want to leave this group? This action cannot be undone.</p>
